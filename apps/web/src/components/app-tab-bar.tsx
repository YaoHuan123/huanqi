...
const tabs = [
  { href: "/write", label: "Write", icon: "write" },
  { href: "/library", label: "Mine", icon: "library" },
  { href: "/matches", label: "Matches", icon: "matches" },
  { href: "/settings", label: "Me", icon: "me" },
] as const;
        ...
        {tabs.map((tab) => {
          ...
          return (
            <Link
              key={tab.href}
              href={tab.href}
              className={active ? "app-shell-tab active" : "app-shell-tab"}
